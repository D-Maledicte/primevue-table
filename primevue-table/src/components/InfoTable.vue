    ...
    <template #header>
      <div class="flex justify-between">
        <div class="flex flex-row justify-between gap-4">
          <Select v-model="sortKey" :options="sortOptions" class="w-52" optionLabel="label"
            placeholder="Ordenar por fecha" @change="onSortChange($event)" />
          <Button type="button" icon="pi pi-filter"
            :pt="{ root: { class: 'my-custom-button-secondary-no-background' } }" outlined @click="toggle"
            v-tooltip.bottom="'Opciones de filtrado'" />
          <Popover ref="popover">
            <div class="flex flex-col justify-between rounded p-0.5 gap-6 mt-6">
              <FloatLabel>
                <MultiSelect id="type_selector" ref="multiselect" v-model="infoTableTypeSelector"
                  :options="logsRepresentatives.tipo" :selectedItemsLabel="'{0} opciones elegidas'" showToggleAll
                  filterPlaceholder="Buscar" :maxSelectedLabels="2" class="w-56">
                  <template #option="slotProps">
                    <div class="flex items-center gap-2">
                      <Tag :value="slotProps.option"
                        v-bind="getSeverityLogs(slotProps.option) == null ? { class: 'severity-null' } : { severity: getSeverityLogs(slotProps.option) }">
                      </Tag>
                    </div>
                  </template>
                  <template #footer>
                    <div class="flex items-end justify-end gap-6">
                      <Button icon="pi pi-trash" text rounded aria-label="Confirm" @click="deleteFilterMultiSelect()"
                        v-tooltip.bottom="'Borrar selección'"></Button>
                      <Button icon="pi pi-check-square" text rounded aria-label="Confirm"
                        @click="applyFilterMultiSelect()" v-tooltip.bottom="'Confirmar selección'"></Button>
                    </div>
                  </template>
                </MultiSelect>
              ...
              <FloatLabel>
                <DatePicker v-model="infoTableDateSelector" selectionMode="range" showButtonBar
                  :hideOnRangeSelection="true" :manualInput="false" inputId="date_selector" class="w-56"
                  placeholder="Desde - Hasta">
                </DatePicker>
                <label for="date_selector">Filtrar por fecha</label>
              </FloatLabel>
              <div class="flex items-end justify-end gap-6">
                <Button icon="pi pi-trash" text rounded aria-label="Close" @click="deleteFilterPopover"
                  v-tooltip.bottom="'Borrar filtros'"></Button>
                <Button icon="pi pi-check-square" text rounded aria-label="Confirm" @click="applyFilterPopover"
                  v-tooltip.bottom="'Aplicar filtros'"></Button>
              </div>
            </div>
          </Popover>
        </div>
        <div>
          ...
          <template v-if="maximizeIconShow == true">
            <Button icon="pi pi-window-maximize" text rounded aria-label="Close" @click="maximizeCallbackDialog()"
              v-tooltip.bottom="maximizeStatus" />
          </template>
        </div>
      </div>
    </template>
              ...
              <div class="flex flex-col md:flex-row justify-between md:items-center flex-1 gap-6">
                ...
                <div class="flex flex-col md:items-end gap-4 max-w-24">
                  <div class="w-full flex justify-center">
                    <Tag icon="pi pi-calendar" :value="formatDateForDisplay(item.fecha_de_movimiento)"
